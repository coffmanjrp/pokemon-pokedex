...
export function useBackgroundPreload({
    ...
    try {
      ...
      Object.keys(cache).forEach((key) => {
        ...
        if (key.includes("GetPokemons")) {
          ...
          const pokemonData =
            queryData?.pokemons ||
            queryData?.pokemonsBasic ||
            queryData?.pokemonsFull;
  ...
  const getGenerationStartTargets = useCallback((): number[] => {
    ...
    generationStarts.forEach((startId) => {
      ...
      for (let i = 0; i < 9; i++) {
        ...
        try {
          const cache = apolloClient.cache.extract();
          const isAlreadyCached = Object.keys(cache).some(
            (key) =>
              ...
              (key.includes("GetPokemons") &&
                (() => {
                  ...
                  const pokemonData =
                    ...
                    queryData?.pokemonsBasic ||
                    queryData?.pokemonsFull;
                  return pokemonData?.edges?.some(
                    (edge: PokemonEdge) => parseInt(edge.node.id) === pokemonId,
                  );
                })()),
          );
          if (!isAlreadyCached) {
  ...
  const getPreloadTargets = useCallback(
    (currentId: number): number[] => {
      ...
      for (
        let i = currentId + 1;
        i <= Math.min(range.max, currentId + 10);
        i++
      ) {
      ...
      if (remaining > 0) {
        targets.push(
          ...generationStartTargets
            .filter((id) => !targets.includes(id) && id !== currentId)
            .slice(0, remaining),
        );
      }
  ...
  const getPopularPokemonInRange = (range: {
    min: number;
    max: number;
    generation: number;
  }): number[] => {
    ...
    const popularByGeneration: { [key: number]: number[] } = {
      ...
      4: [
        389, 392, 395, 445, 448, 461, 466, 467, 477, 480, 481, 482, 483, 484,
        487, 491, 493,
      ], // Starter finals, Garchomp, Lucario, Weavile, Electivire, Magmortar, Dusknoir, lake trio, creation trio, Darkrai, Arceus
      ...
      7: [
        724, 727, 730, 738, 745, 784, 785, 786, 787, 788, 789, 790, 791, 792,
        800, 801, 802,
      ], // Starter finals, Vikavolt, Primarina, Lycanroc, Tapu guardians, Necrozma, Marshadow, Zeraora
      8: [
        812, 815, 818, 823, 830, 884, 888, 889, 890, 891, 892, 893, 894, 895,
        896, 897, 898,
      ], // Starter finals, Corviknight, Gossifleur, Dragapult, Zacian, Zamazenta, Eternatus, Kubfu, Urshifu, Regieleki, Regidrago, Glastrier, Spectrier, Calyrex
      9: [
        906, 909, 912, 1000, 1001, 1002, 1003, 1004, 1007, 1008, 1009, 1010,
        1017, 1024, 1025,
      ], // Paldea starter finals and other popular Pokemon
    };
  ...
  const executePreload = useCallback(
    async (targets: number[], signal: AbortSignal) => {
  ...
  useEffect(() => {
    ...
    const timeoutId = setTimeout(() => {
      ...
      executePreload(targets, controller.signal).finally(() => {
