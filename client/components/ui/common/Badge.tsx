...
export function TypeBadge({ type, displayName, size = 'md', className = '' }: TypeBadgeProps) {
...
export function LevelBadge({ level, variant = 'level', className = '' }: LevelBadgeProps) {
  const variantStyles = {
  ...
  const displayText = typeof level === 'number' 
    ? `Lv. ${level}` 
    : level;
